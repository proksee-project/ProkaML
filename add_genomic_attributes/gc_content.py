...
def metadata_file_read(file):
    df = pd.read_csv(os.path.join(file), sep='\t', keep_default_na=False)
...
def main():
    if len(sys.argv) != 5:
        sys.exit('''
        Command usage: python gc_content.py EMAIL API_KEY FILE_NAME OUTPUT_DIRECTORY.
        Need to pass 4 arguments corresponding to your email, ncbi api key, file name with annotated metadata
    ...
    outfile = filename.split('.')[0] + '_added_attributes.txt'
