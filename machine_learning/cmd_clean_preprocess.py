...
from preprocess_metadata import GroupedNormalizing
...
normalized_dataframe_file = open('well_represented_species_metadata_normalized.txt', 'w')
...
for individual_species_dataframe in valid_species_list:
	...
	species_normalized_dataframe = grouped_normalization.normalize_dataframe(individual_species_dataframe)

	if species_count == 0:
		header = True
	else:
		header = False
	species_normalized_dataframe.to_csv(normalized_dataframe_file, sep='\t', mode='a', header=header, index=False)
	species_count += 1
